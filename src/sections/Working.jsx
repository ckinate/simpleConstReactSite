...
export default function Working() {
  return (
    ...
    <motion.div initial='hidden' whileInView='visible' variants={slideUpVariants}
      ...
      <motion.h1 variants={slideUpVariants} className=' text-black uppercase text-[40px] font-bold text-center'> HOW's IS WORKING </motion.h1>
      <motion.div variants={slideUpVariants} className='w-[120px] h-[6px]  bg-yellow-500'></motion.div>
      {/* make dive for service mapping from export.js file */}
          ...
          className='w-full grid lg:grid-cols-4 grid-cols-1 justify-center items-start gap-[20px] mt-[30px]'>
          {
            planning.map((item, index) => (
              <div key={index} className='flex flex-col justify-center items-center gap-5 border-2 border-yellow-500 rounded-md p-6'>
                ...
                <p className='item-[20px] text-center text-gray-600'>{item.about }</p>
